 ...
 */
	...
	$settingValues = $result->fetch_all();
	$submit = cOut($settingValues[0][0]);
	$rssMax = cOut($settingValues[1][0]);
	$passApprove = cOut($settingValues[2][0]);
	$passDecline = cOut($settingValues[3][0]);
	$driving = cOut($settingValues[4][0]);
	$weather = cOut($settingValues[5][0]);
	$mostPopular = cOut($settingValues[6][0]);
	$browsePast = cOut($settingValues[7][0]);
	$maxShow = cOut($settingValues[8][0]);
	$fillMax = cOut($settingValues[9][0]);
	$dateFormat = cOut($settingValues[10][0]);
	$showTime = cOut($settingValues[11][0]);
	$state = cOut($settingValues[12][0]);
	$calStartDay = cOut($settingValues[13][0]);
	$timeFormat = cOut($settingValues[14][0]);
	$popDateFormat = cOut($settingValues[15][0]);
	$passAge = cOut($settingValues[16][0]);
	$langType = cOut($settingValues[17][0]);
	$userCat = cOut($settingValues[18][0]);
	$WYSIWYG = cOut($settingValues[19][0]);
	$timeInput = cOut($settingValues[20][0]);
	$captchas = explode(",", cOut($settingValues[21][0]));
	$series = cOut($settingValues[22][0]);
	$browseType = cOut($settingValues[23][0]);
	$timezoneOffset = cOut($settingValues[24][0]);
	$subLimit = cOut($settingValues[25][0]);
	$stats = cOut($settingValues[26][0]);
	$float = cOut($settingValues[27][0]);
	$searchWindow = cOut($settingValues[28][0]) + 1;
	$pubNews = cOut($settingValues[29][0]);
	$jssMax = cOut($settingValues[30][0]);
	$capType = cOut($settingValues[31][0]);
	$maxNew = cOut($settingValues[32][0]);
	$reCapPub = cOut($settingValues[33][0]);
	$reCapPriv = cOut($settingValues[34][0]);
	$locSelect = cOut($settingValues[35][0]);
	$mailMethod = cOut($settingValues[36][0]);
	$mailHost = cOut($settingValues[37][0]);
	$mailPort = cOut($settingValues[38][0]);
	$mailSecure = cOut($settingValues[39][0]);
	$mailUser = cOut($settingValues[40][0]);
	$mailPass = base64_decode(cOut($settingValues[41][0]));
	$mailAuth = cOut($settingValues[42][0]);
	$mailAddress = cOut($settingValues[43][0]);
	$mailName = cOut($settingValues[44][0]);
	$loginTries = cOut($settingValues[45][0]);
	$batchSize = cOut($settingValues[46][0]);
	$batchDelay = cOut($settingValues[47][0]);
	$sitemap = cOut($settingValues[48][0]);
	$iCalMax = cOut($settingValues[49][0]);
	$iCalRef = cOut($settingValues[50][0]);
	$reCapStyle = cOut($settingValues[51][0]);
	$passStr = cOut($settingValues[52][0]);
	$mc_select = cOut($settingValues[53][0]);
	$mc_dow = cOut($settingValues[54][0]);
	$rssStatus = cOut($settingValues[55][0]);
	$rssTrunc = cOut($settingValues[56][0]);
	$iCalStatus = cOut($settingValues[57][0]);
	$iCalTrunc = cOut($settingValues[58][0]);
	$sendPast = cOut($settingValues[59][0]);
